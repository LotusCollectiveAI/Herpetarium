...
3. **Chat rework** (Codex, per founder feedback): global human chat always-on (drop the encryptor/observer hard mutes for *humans*, keep the keyword scanner), add a real `team:*` channel, and a per-game `teamChatVisibility: open | private` setting — the two worlds the founder described: opponents can read your team channel, or they cannot. The substrate encodes the setting in the observation so bots know what the opponent can hear, and Herpetarium can run the same two conditions as experiments (T1's "private vs public deliberation" proposal, productized). Research may study intermediate conditions (e.g. delayed reveal) on its own side later; the product slice ships two.
...
5. **Run the missing experiments in Herpetarium** against the fixed (P4-C) rules with the reliability guards now in `modelHealth.ts`/`modelValidation.ts`: K-level vs advanced vs baseline (the SPEC_WEEK3 design, finally), candidate-clue verification, team-chat visibility A/B (open vs private). The verification stage is no longer hypothetical: the blind-inversion probe caught all six production clues for a sixth of a cent in under five seconds, which means inversion can run **per clue submission at play time** (generate candidates → invert blind → discard flagged → submit) as well as in evaluation. First evaluator milestone: calibrate the false-positive rate on known-good clues and version the protocol as `blind-inversion@0.1` (provisional flag threshold 0.60 = minimum observed on known-transparent clues). Artifacts that win get names and go to The Table **through a promotion gate**: seating-as-validated requires a typed immutable `EvaluationRecord` binding the exact artifact (id + content hash), model route, compiler version, and candidate policy to a named protocol version, held-out matched tests, a verdict, and an explicit scope — a nonempty pointer list proves nothing. Seeds may be seated before promotion only by explicit `allowUnvalidated` policy, with traces marked unvalidated.
...
## 7. Founder judgment questions

1. **Team-chat visibility default.** Two worlds ship: `open` (opponents read your team channel — the authentic around-the-table experience, kept survivable by the existing intercept-before-decode ordering) or `private` (competitive integrity; bots and humans deliberate freely). My recommendation: default `open` for family/social games, `private` selectable for competitive play — and the open-vs-private A/B is the highest-value first experiment on The Table traces.
2. **Should bots reason out loud at all?** The current design makes bot strategy talk public-only. With a real team channel, do bot decode/intercept deliberations move to team chat (subject to the visibility setting) and table-talk stays purely social? I believe yes — it dissolves most of the fence lattice in `prompts.ts:139-208` rather than patching it an eighth time.
3. **Encryptor mute for humans.** Keep poker-face muting (rules purity), or let human encryptors banter with the scanner as the only guard (social product)? Feedback says humans must always be able to talk; I'd un-mute with scanner + a UI nudge, and keep the mute for *bot* encryptors.
4. **Where does artifact authority live?** Proposal: Herpetarium owns training/eval and artifact minting; The Table consumes read-only artifact bundles and produces traces; Extrospection records the ledger of named artifacts + provenance. Any objection to research being the sole minting authority?
5. **Model spend for the missing experiments.** The K-level and visibility A/Bs at honest power (12–16 games/cell, P4-C rules) are roughly a T1-sized spend (~$40–80 at T1 prices) per experiment. Green-light now, or after the first artifact-seated human game?
6. **DeepSeek routing for experiments.** Two viable treatments, not one: (a) **canonical cheap pinned route** — OpenRouter dated slug `deepseek/deepseek-v4-flash-0731` with one pinned upstream (DeepInfra at $0.09/$0.18), `allow_fallbacks:false`, `require_parameters:true`, persisting canonical slug, served model, upstream, route attempt, effort, and full request/response; (b) **provenance canary** — the direct DeepSeek API's mutable `deepseek-v4-flash` alias, which buys official serving provenance, the response fingerprint, large output windows, and much cheaper cache-read, stamped with an alias-epoch per trace. Run (a) as the experiment workhorse and (b) as a periodic canary; diffing them detects both upstream serving drift and future silent alias mutations. Any objection to funding both lanes in the model registry?
7. **Clue format.** The 120-char phrase allowance is what made definition-clues possible (§2.0). Options: (a) single-word clues only (classic constraint, forces abstraction, matches Herpetarium's trained regime), (b) keep phrases but hold bots to single words via the artifact's execution guidance, (c) keep phrases and rely on a transparency evaluator to veto definition-shaped clues. I recommend (b) now and (c) once the evaluator exists — human players keep expressive freedom while bots stop being handed the definition-clue trap.
