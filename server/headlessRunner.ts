...
async function processClues(
  ...
  for (const team of ["amber", "blue"] as const) {
    ...
    const teamNotes = matchConfig?.scratchNotesByTeam?.[team] ?? scratchNotesMap?.[noteKey];
    const clueParams = { keywords, targetCode: code, history, scratchNotes: teamNotes, ablations, systemPromptOverride: teamSystemPrompts?.[team] };
...
async function processGuesses(
  ...
  for (const team of ["amber", "blue"] as const) {
    ...
    const teamNotes = matchConfig?.scratchNotesByTeam?.[team] ?? scratchNotesMap?.[noteKey];
    const guessParams = { keywords, clues, history, scratchNotes: teamNotes, ablations, systemPromptOverride: teamSystemPrompts?.[team] };
    const { result: callResult, timedOut } = await withTimeout(
      config.timeoutMs,
...
async function processInterceptions(
  ...
  for (const team of ["amber", "blue"] as const) {
    ...
    const teamNotes = matchConfig?.scratchNotesByTeam?.[team] ?? scratchNotesMap?.[noteKey];
    const interceptParams = { clues, history, scratchNotes: teamNotes, ablations, systemPromptOverride: teamSystemPrompts?.[team] };
    const { result: callResult, timedOut } = await withTimeout(
      config.timeoutMs,
...
export async function runHeadlessMatch(
  ...
  while (game.phase !== "game_over") {
    ...
    game = await processClues(game, matchId, qualityState, scratchNotesMap, ablations, teamSystemPrompts, healthTracker, config);
    ...
    if (teamSize === 3) {
      ...
      game = await processGuesses(game, matchId, qualityState, scratchNotesMap, ablations, teamSystemPrompts, healthTracker, config);
      ...
      game = await processInterceptions(game, matchId, qualityState, scratchNotesMap, ablations, teamSystemPrompts, healthTracker, config);
    }
