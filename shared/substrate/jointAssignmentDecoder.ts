 ...
 * commit vendors this module but leaves the live runtime excerpt unchanged.
 *
 ...
 * envelope and one authoritative CodeGuess contract to both arms.
 */
...
/**
 * Provider-free, neutral experiment compiler. It intentionally does not
 * replace or mutate the existing joint-assignment compiler above, and it does
 * not claim production parity with compileJointAssignmentDecoderPrompt.
 * Both comparison arms receive this exact neutral role envelope, Observation
 * v0.2 renderer, and CodeGuess contract; only the policy instruction text
 * changes.
 *
 * This experiment-only compiler stays adjacent to the production compiler
 * because it reuses the same private observation renderers and action
 * contract. Extracting it now would either duplicate those authorities or
 * widen their public API solely for a provider-free smoke scaffold.
 */
export const PAIRED_DECODER_POLICY_COMPILER_ID =
  "paired-decoder-policy-compiler@0.1.0";

export const PAIRED_DECODER_POLICY_SECTION_HEADING =
...
const PAIRED_DECODER_POLICY_COMPILER_SOURCE = {
  ...
  actionContract: ACTION_CONTRACTS.decoder,
} as const;
 ...
 * may send only systemPrompt, taskPrompt, and the action contract appropriate
 * to its API; it must never serialize this record wholesale.
 */
